...
go build ${GOFLAGS} github.com/RarimoVoting/certificate-transparency-go/trillian/integration/ct_hammer
ct_prep_test 1
...
set +e
./ct_hammer --log_config "${CT_CFG}" --ct_http_servers=${CT_SERVERS} --mmd=30s --testdata_dir=$(go list -f '{{ .Dir }}' github.com/RarimoVoting/certificate-transparency-go)/trillian/testdata --metrics_endpoint="localhost:${metrics_port}" --logtostderr ${HAMMER_OPTS}
